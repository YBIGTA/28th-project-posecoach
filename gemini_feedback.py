...
반드시 아래 구조대로 상세하게 작성하세요 (최소 800자 이상):

## 1. 총평
- 전반적인 운동 수행 평가 (점수, 등급 언급)
- 잘한 점을 구체적으로 칭찬

## 2. Phase별 상세 분석
- 각 Phase(ready, ascending, descending, top, bottom 등)별로 소제목을 나누어 분석
- 해당 Phase의 평균 점수와 프레임 수를 언급
- 점수가 낮은 Phase는 왜 낮은지 구체적 원인 분석
...
## 5. 개선 우선순위 및 연습 팁
- 가장 먼저 개선해야 할 점 3가지를 우선순위로 제시
- 각 항목에 대한 연습 방법 제안

피드백 작성 원칙:
1. 수치 데이터(각도, 점수 등)를 직접 언급하여 신뢰감을 높이세요.
2. "더 깊이 내려가세요" 같은 단순 지적보다 "팔꿈치 각도가 {값}°인데, 90° 아래로 내려가면 가슴근육 자극이 증가합니다" 처럼 이유를 설명하세요.
3. 마크다운 형식(##, **볼드**, - 목록)을 적극 활용하세요.
4. 절대 요약하지 마세요. 모든 데이터를 빠짐없이 분석하여 풍부하고 상세한 피드백을 작성하세요.
"""
...
시스템 프롬프트에서 지정한 5개 섹션(총평, Phase별 상세 분석, 주요 오류 및 교정 방법, DTW 유사도 분석, 개선 우선순위 및 연습 팁) 구조를 반드시 따르세요.
모든 Phase와 오류 데이터를 빠짐없이 분석하고, 최소 800자 이상으로 상세하게 작성하세요.
"""
