...
def _extract_base_features(frame_files, preloaded_grays=None):
    n = len(frame_files)
    ...
    def _get_gray(idx):
        if preloaded_grays is not None:
            return preloaded_grays[idx]
        return _load_gray_small(frame_files[idx])
...
def build_feature_matrix(frame_files, temporal_window=5, preloaded_grays=None):
    """Build shared feature matrix for training/inference."""
    base = _extract_base_features(frame_files, preloaded_grays=preloaded_grays)
    if base.shape[0] == 0:
        return base
...
def _rule_based_indices(
    ...
    min_keep_ratio=0.08,
    preloaded_grays=None,
):
    if not frame_files:
        return set()

    feats = _extract_base_features(frame_files, preloaded_grays=preloaded_grays)
...
def _ml_based_indices(
    ...
    feats = build_feature_matrix(frame_files, preloaded_grays=preloaded_grays)
    if feats.shape[0] == 0:
        return set(), ""
