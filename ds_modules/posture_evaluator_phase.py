...
class PushUpEvaluator:
    ...
    CHEST_MOVEMENT_THRESHOLD = 0.00024
...
class PullUpEvaluator:
    ...
    SHOULDER_PACKING_THRESHOLD = 0.005

    # 팔꿈치 벌림:
    #   Prinold & Bull (2016) — 와이드 그립 시 견관절 90° 외전 + 45° 외회전
    #     → 견봉하 충돌 위험 증가. 견갑면 이탈 < 28-30° 권장
    #   Youdas et al. (2010) — 풀업 팔꿈치 ROM 93.4 ± 14.6°,
    #     표준 그립 = 어깨너비 ~ 1.5배
    #   Lauder & Giannasi (2023) "Biomechanical characterisation of the pull-up",
    #     Sport Sciences for Health 19:1251-1265
    #     — 과도한 팔꿈치 벌림을 안전 위험 보상 동작으로 분류
    #   AI Hub Cohen's d |d|=0.31 (small~medium)
    ELBOW_FLARE_RATIO = 1.3
    ...
    def _check_elbow_flare(self, npts, details, errors) -> bool:
        ...
        if ratio <= self.ELBOW_FLARE_RATIO:
            details["elbow_direction"] = {"value": round(ratio, 2), "status": "ok", "feedback": "팔꿈치 방향 양호"}
            return True
        details["elbow_direction"] = {"value": round(ratio, 2), "status": "error", "feedback": "팔꿈치를 몸쪽으로 당기세요"}
    ...
    def _evaluate_ascending(self, npts: Dict) -> Dict:
        """
        올라가는 중 평가: 고개 제외, 자세 유지 체크

        가중치: shoulder_packing(0.20), elbow_flare(0.19), body_sway(0.08)
        """
        errors: List[str] = []
        details: Dict = {}
        checks: Dict[str, bool] = {}
    ...
    def _evaluate_bottom(self, npts: Dict) -> Dict:
        ...
        가중치: shoulder_packing(0.20), body_sway(0.08)
        """
        errors: List[str] = []
        details: Dict = {}
        checks: Dict[str, bool] = {}
